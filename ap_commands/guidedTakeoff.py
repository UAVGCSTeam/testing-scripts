...
try: 
    ...
    if SERIAL: 
        ...
        mavObj = mavutil.mavlink_connection(port, baud=BAUD)
    ...
    msg = mavObj.recv_match(type='HEARTBEAT', blocking=True)
    current_mode = mavutil.mode_string_v10(msg)
    print(f"[{time.strftime("%I:%M:%S %p")}] … Current mode: {current_mode}")
    ...
    msg = mavObj.recv_match(type='HEARTBEAT', blocking=True, timeout=10)
    if msg:
    ...
    print(f"[{time.strftime("%I:%M:%S %p")}] {'✔' if armed == 128 else '✖︎'} Vehicle {'' if armed == 128 else 'not '}armed")
    ...
    print(f"[{time.strftime("%I:%M:%S %p")}] {'✔' if ack.result == 0 else '✖︎'} ARM ACK:", ack.result)
    ...
    mavObj.mav.command_long_send(
        ...
        10            # param7: altitude (meters AGL or AMSL per firmware config)
    )
    print(f"[{time.strftime("%I:%M:%S %p")}] … Takeoff command sent")
    ack = mavObj.recv_match(type='COMMAND_ACK', blocking=True)
    print(f"[{time.strftime("%I:%M:%S %p")}] {'✔' if ack.result == 0 else '✖︎'} TAKEOFF ACK: {ack.result}")
